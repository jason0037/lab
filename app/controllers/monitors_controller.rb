  ...
  def get_realtime_data
    ...
    case table_name
      ...
      when 'B000001'
        ...
        sql = "select value/2 as value,read_at from #{table_name}_reading where source=#{source} and point_id='000000'
        and read_at>= '#{read_at.to_s}' order by id desc limit 0,1"
...
<apply toobject='Realtimevalue' styles='captionFont' />
  ...
  def behaviour_data_history
    ...
    [1,2,3,4].each do |source|
       sql = "select value / 2 as value ,minute as read_at from #{table_name}_minute where source=#{source} and point_id='000000' and minute >= '#{start_time}' and minute<='#{end_time}'"
   #     str = str + sql
        results = ActiveRecord::Base.connection.execute(sql)
        data_str1 = ""
        last_time = ""
...
<apply toobject='Realtimevalue' styles='captionFont' />
  ...
  def stadard_deviation
    ...
    datasets = ""
    cats_str = ""
      sql = "select value ,minute,source from #{table_name}_minute where point_id='#{point}' and minute >= '#{start_time}' and minute<='#{end_time}' order by minute,source"
      results = ActiveRecord::Base.connection.execute(sql)
      data_str1 = ""
      minute= ""
      results.each(:as => :hash) do |row|
        if minute == row["minute"][8..12]
          data_str1 += ",#{row["value"].to_i}"
        else
          minute = row["minute"][8..12]
          times = minute[0..1] + ":"+ minute[2..3]
          data_str1 += "'/><set value='#{row["value"].to_i}"
          cats_str += "<category label='#{times}'/>"
        end
      end
      ...
      datasets = datasets + "<dataset>#{data_str1.sub('\'/>','')}</dataset>"
...
snumbersuffix='' setadaptiveymin='1' setadaptivesymin='1' xaxisname='#{xaxisname}'
showrealtimevalue='1' labeldisplay='Rotate' slantlabels='1' numdisplaysets='#{displayNum}'
labelstep='1' pyaxisminvalue='0' pyaxismaxvalue='10' syaxisminvalue='0' syaxismaxvalue='10' #{export_str}>
#{categories} #{datasets}
<styles>
<definition>
<style type='font' name='captionFont' size='14' />
...
<trendlines></trendlines>
  ...
  def behaviour_data
    ...
    table_name = LabEquipmentMapping.find_by_equipment_code(equipment_code).table_name
    end_time = Time.now.strftime('%Y%m%d%H%M%S')
    #start_time = (Time.now - 2.minutes).strftime('%Y%m%d%H%M%S')
    ...
    sql = "select value / 2 as value,read_at from #{table_name}_reading where source=#{source} and point_id='000000' and read_at >= '#{start_time}' and read_at<=#{end_time}"
    results = ActiveRecord::Base.connection.execute(sql)
